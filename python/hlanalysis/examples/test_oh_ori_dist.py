...
def main():
    ...
    water_mol_map = {(ox, h1x, h2x): idx for idx, (ox, h1x, h2x) in enumerate((ox_i, ox_i+1, ox_i+2) for ox_i in water_oxygen_indices)}
    ...
    # -----------------------------
    oh_angles = orient_dist.oh_bond.cal_oh_bond_angles(oh_bond_map=oh_pair_map,
    ...
    angle_edges, angle_centers = histogram.generate_bins(0, 180 + d_angle, d_angle)

    # Bin orientation data along z
    oh_angles_bins_along_z = orient_dist.oh_bond.angle_bins_along_normal(
                                                     all_angles=oh_angles,
                                                     bins_normal=z_edges,
                                                     centers_normal=z_centers,
                                                     parallel=True,
                                                     dir=output_dir,
                                                     start_ts=start_ts,
                                                     end_ts=end_ts,
                                                     interval_ts=ts_interval)
    # Accumulate angle distributions per z-bin
    oh_angles_avg_bins_along_z = orient_dist.oh_bond.accumulate_angle_bins_along_normal(
                                                    all_angles_bins=oh_angles_bins_along_z,
                                                    centers_normal=z_centers,
                                                    bins_angles=angle_edges,
                                                    centers_angles=angle_centers
                                                    )
    oh_angles_avg_bins_along_z = np.array(oh_angles_avg_bins_along_z)

    # ---------------------------------------------------------------------------------------------------------------- #
    # Save Orientation Distribution
    # ---------------------------------------------------------------------------------------------------------------- #
    output_data = np.zeros((len(z_centers), len(angle_centers) + 1))  
    output_data[:, 0] = z_centers  
    output_data[:, 1:] = oh_angles_avg_bins_along_z  
    np.savetxt(
        os.path.join(ori_result_dir, "oh_bond_orient_dist.txt"),
        output_data,
        fmt="%.6f",
        header="z_center " + " ".join(f"angle_{i}" for i in angle_centers),
    )
    
    # ---------------------------------------------------------------------------------------------------------------- #
    # Visualization
    # ---------------------------------------------------------------------------------------------------------------- #
    oh_angles_avg_bins_along_z /= num_frames
    oh_angles_avg_prob_dist = oh_angles_avg_bins_along_z / np.sum(oh_angles_avg_bins_along_z)
    ...
    def plot_angle_distribution(distribution, angle_centers, z_centers, save=False):
        ...
        plt.xlabel(r'$\psi_{OH}$ (°)', fontsize=20)
        ...
        if save:
            plt.savefig(os.path.join(ori_result_dir, "oh_ori_dist.png"))
        else:
            plt.show()

    plot_angle_distribution(oh_angles_avg_prob_dist, angle_centers, z_centers, save=True)
